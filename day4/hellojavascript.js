...
console.log(foo([1,3,5,7,9]));  //返回sum这个函数
console.log(foo([1,3,5,7,9])());  //返回执行的结果*/
//在这个例子中，我们在foo()函数中定义了sum函数，并且sum函数可以引用外部函数foo()函数的局部变量和参数。
//在foo()函数返回的函数sum中，保存了相关参数和变量，这种成为“闭包”
//注意一点：调用函数foo()时，每次返回的函数sum()都是一个新的函数，即使传入的是相同的参数
